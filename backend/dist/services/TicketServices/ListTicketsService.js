...
const ShowUserService_1 = __importDefault(
  require("../UserServices/ShowUserService")
);
const ListTicketsService = ({
  searchParam = "",
  pageNumber = "1",
  queueIds,
  status,
    ...
    if (searchParam) {
      ...
      includeCondition = [
        ...includeCondition,
        {
          ...
          attributes: ["id", "body"],
          where: {
            body: sequelize_1.where(
              sequelize_1.fn("LOWER", sequelize_1.col("body")),
              "LIKE",
      ...
      whereCondition = Object.assign(Object.assign({}, whereCondition), {
        [sequelize_1.Op.or]: [
          {
            "$contact.name$": sequelize_1.where(
              sequelize_1.fn("LOWER", sequelize_1.col("contact.name")),
              "LIKE",
              `%${sanitizedSearchParam}%`
            )
          },
          ...
          {
            "$message.body$": sequelize_1.where(
              sequelize_1.fn("LOWER", sequelize_1.col("body")),
              "LIKE",
              `%${sanitizedSearchParam}%`
            )
      ...
      whereCondition = {
        createdAt: {
          [sequelize_1.Op.between]: [
            +date_fns_1.startOfDay(date_fns_1.parseISO(date)),
            +date_fns_1.endOfDay(date_fns_1.parseISO(date))
          ]
        }
      };
    }
